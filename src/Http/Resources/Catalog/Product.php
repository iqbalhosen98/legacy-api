...
namespace Mrpath\API\Http\Resources\Catalog;
...
class Product extends JsonResource
{
    ...
    {
        $this->productReviewHelper = app('Mrpath\Product\Helpers\Review');

        $this->wishlistHelper = app('Mrpath\Customer\Helpers\Wishlist');
        
        $this->productViewHelper = app('Mrpath\Product\Helpers\View');
    ...
    private function allProductExtraInfo()
    {
        ...
        return [
            /* grouped product */
            $this->mergeWhen(
                $productTypeInstance instanceof \Mrpath\Product\Type\Grouped,
                $product->type == 'grouped'
            ...
            /* bundle product */
            $this->mergeWhen(
                $productTypeInstance instanceof \Mrpath\Product\Type\Bundle,
                $product->type == 'bundle'
            ...
            $this->mergeWhen(
                $productTypeInstance instanceof \Mrpath\Product\Type\Configurable,
            ...
            $this->mergeWhen(
                $productTypeInstance instanceof \Mrpath\Product\Type\Downloadable,
     ...
     *
     * @param  \Mrpath\Product\Models\Product
     * @return array
     */
    ...
    private function getBundleProductInfo($product)
    {
        return [
            ...
            'bundle_options' => app('Mrpath\Product\Helpers\BundleOption')->getBundleConfig($product)
        ];
    }
    ...
    private function getBookingProductInfo($product)
    {
        ...
        if ($bookingProduct->type == 'appointment') {
            $bookingSlotHelper = app('\Mrpath\BookingProduct\Helpers\AppointmentSlot');
        ...
        if ($bookingProduct->type == 'event') {
            $bookingSlotHelper = app('\Mrpath\BookingProduct\Helpers\EventTicket');
        ...
        if ($bookingProduct->type == 'table') {
            $bookingSlotHelper = app('\Mrpath\BookingProduct\Helpers\TableSlot');
