...
namespace Mrpath\API\Http\Resources\Checkout;

use Illuminate\Http\Resources\Json\JsonResource;
use Mrpath\API\Http\Resources\Core\Channel as ChannelResource;
...
class Cart extends JsonResource
{
    ...
    public function toArray($request): array
    {
        ...
        return [
            ...
            'grand_total'                        => (float) core()->convertPrice($this->base_grand_total, $currencyCode),
            'formated_grand_total'               => (string) core()->formatPrice(core()->convertPrice($this->base_grand_total, $currencyCode), $currencyCode),
            'base_grand_total'                   => (float) $this->base_grand_total,
            'formated_base_grand_total'          => (string) core()->formatBasePrice($this->base_grand_total),
            'sub_total'                          => (float) core()->convertPrice($this->base_sub_total, $currencyCode),
            'formated_sub_total'                 => (string) core()->formatPrice(core()->convertPrice($this->base_sub_total, $currencyCode), $currencyCode),
            'base_sub_total'                     => (float) $this->base_sub_total,
            'formated_base_sub_total'            => (string) core()->formatBasePrice($this->base_sub_total),
            'tax_total'                          => (float) core()->convertPrice($this->base_tax_total, $currencyCode),
            'formated_tax_total'                 => (string) core()->formatPrice(core()->convertPrice($this->base_tax_total, $currencyCode), $currencyCode),
            'base_tax_total'                     => (float) $this->base_tax_total,
            'formated_base_tax_total'            => (string) core()->formatBasePrice($this->base_tax_total),
            'discount'                           => (float) core()->convertPrice($this->base_discount_amount, $currencyCode),
            'formated_discount'                  => (string) core()->formatPrice(core()->convertPrice($this->base_discount_amount, $currencyCode), $currencyCode),
            'base_discount'                      => (float) $this->base_discount_amount,
            'formated_base_discount'             => (string) core()->formatBasePrice($this->base_discount_amount),
