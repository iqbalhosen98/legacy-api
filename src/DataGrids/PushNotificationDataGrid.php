...
namespace Mrpath\API\DataGrids;
...
class PushNotificationDataGrid extends DataGrid
{
    ...
    public function addColumns()
    {
        ...
        $this->addColumn([
            ...
            'wrapper'       => function($row) {
                $channelNames = '';
                $notificationTranslations = app('Mrpath\API\Repositories\NotificationTranslationRepository')->where(['push_notification_id' => $row->notification_id])->groupBy('push_notification_id', 'channel')->pluck('channel')->toArray();

                if ($notificationTranslations) {
                    $channels = app('Mrpath\Core\Repositories\ChannelRepository')->whereIn('code', $notificationTranslations)->get();
